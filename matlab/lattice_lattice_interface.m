...
disp = zeros(int32((-left_y+right_y)/a),int32((-left_x+right_x)/a));
vel = zeros(int32((-left_y+right_y)/a),int32((-left_x+right_x)/a));
...
surf(X,Y,disp,'FaceAlpha',0.9,'EdgeAlpha',0.5);
plot3(x_coord,y_coord,max(max(disp))*ones(1,length(x_coord)),'LineWidth',2,'Color','Black')
...
for t=times
    for ind_x=1:length(num_x)
        for ind_y=1:length(num_y)
            ...
            vel(ind_y,ind_x) = vel(ind_y,ind_x) + c/mass(ind_y, ind_x)*(disp(iy_m, ind_x)+disp(iy_p, ind_x)+disp(ind_y, ix_m)+disp(ind_y, ix_p)-4*disp(ind_y, ind_x))*dt;
